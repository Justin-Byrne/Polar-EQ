...
const ELEMENTS =
{
    ...
    btnInit:     document.getElementById ( "btnInit" )     as HTMLButtonElement,
    btnMic:      document.getElementById ( "btnMic" )      as HTMLButtonElement,
    ...
    chkGlow:     document.getElementById ( "chkGlow" )     as HTMLInputElement,
...
function _wireUI ( )
{
    ELEMENTS.btnInit.addEventListener ( "click", ( ) =>
    {
        _usingAudio = false;
        _setStatus ( false );
        _drawGrid ( );
    ...
    ELEMENTS.btnMic.addEventListener ( "click", ( ) => startAudio ( ) );

    ELEMENTS.fileAudio.addEventListener ( "change", async ( element ) =>
    {
        ...
        if ( ! _audioEnvironment )
            ...
            _audioEnvironment = await initAudio ( parseInt ( ELEMENTS.fft.value, 10 ), false );
...
////    FFT SLIDER LOGIC (DYNAMIC ANALYZER REINIT + SAFE POWER-OF-TWO)    /////
    ...
    ELEMENTS.fft.addEventListener ( "input", ( ) =>
    {
        const _raw        = parseInt ( ELEMENTS.fft.value, 10 );
        ...
        ELEMENTS.fft.value = _fftSize.toString ( );
    ...
    ELEMENTS.btnExport.addEventListener ( "click", _exportShapes );

    ELEMENTS.btnSelfTest.addEventListener ( "click", ( ) =>
    {
        try
        {
